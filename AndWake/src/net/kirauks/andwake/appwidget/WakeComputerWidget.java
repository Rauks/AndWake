...
public class WakeComputerWidget extends AppWidgetProvider {
	...
	private static Computer getTarget(Context context, int appWidgetId) {
		...
		long targetId = prefs.getLong(WakeComputerWidget.PREFERENCES_TAG
				+ appWidgetId, 0);
		...
		final List<Computer> computers = db.getComputerDataSource()
				.getComputers(new long[] { targetId });
	...
	static void updateAppWidget(Context context,
		...
		views.setTextViewText(R.id.appwidget_computer_address,
				target.getAddress());
		views.setTextViewText(R.id.appwidget_computer_port,
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (WakeComputerWidget.ACTION_WAKE_COMPUTER.equals(intent.getAction())) {
			...
			Computer target = intent
					.getParcelableExtra(WakeComputerWidget.INTENT_WAKE_COMPUTER);
